...
RSpec.describe FetchAudioEpisodeJob, type: :job do
  ...
  xit 'should save media' do
    expect do
      perform_job
    end.to change { podcast.episodes.count }.by(1)
  end

  xit 'should remove temp file' do
    perform_job
    expect(File.exist?(temp_audio_file_path)).to eq false
  end
  ...
  describe 'new episode' do
    ...
    xit { is_expected.to be_present }

    xit 'should have media' do
