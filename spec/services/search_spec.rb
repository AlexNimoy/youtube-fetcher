...
RSpec.describe Search do
  describe 'search results' do
    ...
    context 'when search "mos"' do
      ...
      let!(:podcast_moscow) { create(:podcast, title: 'Moscow') }
      let!(:podcast_london) { create(:podcast, title: 'London') }

      it { is_expected.to include podcast_moscow.origin_id }
      it { is_expected.not_to include podcast_london.origin_id }

      let!(:episode_moscow_news) { create(:episode, title: 'Moscow News') }
      let!(:episode_peter_news) { create(:episode, title: 'Peter News') }
      ...
      context 'russian query' do
        let(:query) { 'плюш' }
        let!(:episode_plushki) { create(:episode, title: 'Плюшки Ярослава Мудрого') }
